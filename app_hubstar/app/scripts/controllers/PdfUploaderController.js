...
//    },
    ...
    {
        if (this.get('pdfArray').get('length')>0) {
            this.get('controllers.applicationFeedback').statusObserver(null, "You can only upload one pdf at a time");
        } else {
            ...
            if (type === "pdf") {
                ...
                profile.store.save();
                this.get("controllers.profile").set("profilePdfStatistics", this.get("controllers.profile").get("profilePdfStatistics")+1);
            }
            else
            {
