...
//        var lastposition = HubStar.get("scrollCollectionPosition");
        ...
        changeCollectionCover: function(id, collection_id, HubStarModel, article) {
            ...
            this.send("dropdownPhotoSetting", id);
            ...
            for (var i = 0; i < userOrprofile.get("length"); i++) {
        ...
        removeCollectedItem: function(collectionID, itemID, type)
        {
            ...
            this.send("dropdownPhotoSetting", itemID);
            if (this.get('willDelete')) {
                ...
                if (this.get("type") === "user") {
                    ...
                    for (var j = 0; j < this.get('contentData').length; j++) {
                        if (this.get('contentData').objectAt(j).get('id') === this.get('itemID')) {
                            tempItem = this.get('contentData').objectAt(j);
                            for (var i = 0; i < currentUser.get('collections').get('length'); i++) {
                                if (currentUser.get('collections').objectAt(i).get('id') === this.get('collectionID'))
                                {
                                    ...
                                    data[0] = this.get('itemID');
                                    data[1] = currentCollection.get("id");
                                    data[2] = currentCollection.get('optional');
                                    data = JSON.stringify(data);
                                    requiredBackEnd('collections', 'deleteUserCollection', data, 'POST', function(params) {
                                        currentCollection.set('collection_ids', params);
                                        currentCollection.save();
                                    });
                                    this.get('contentData').removeObject(tempItem);
                                    break;
                                }
                            }
                            break;
                ...
                else
                {
                    ...
                    for (var i = 0; i < this.get('contentData').length; i++) {
                        if (this.get('contentData').objectAt(i).get('id') === this.get('itemID')) {
                            tempItem = this.get('contentData').objectAt(i);
...
//                                            delResult = delResult.substr(0, delResult.length - 1);
                                            ...
                                            data[2] = currentCollection.get('optional');
                                            data = JSON.stringify(data);
                                            requiredBackEnd('collections', 'deleteProfileCollection', data, 'POST', function(params) {
                                                currentCollection.set('collection_ids', params);
                                                currentCollection.save();
                                            });
